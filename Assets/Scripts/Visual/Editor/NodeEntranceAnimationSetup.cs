...
{
    /// <summary>
    /// 一键把 NodeEntranceAnimation 挂到所有节点根：
    ...
    /// 之后放置/生成的节点都从 prefab 实例化，自动获得入场动画。
    /// </summary>
    public static class NodeEntranceAnimationSetup
    {
        ...
        [MenuItem("CUC260905/Visual/Apply Node Entrance Animation")]
        public static void ApplyAll()
        {
            ...
            Debug.Log($"NodeEntranceAnimation 装配完成：新增组件 {added} 处（幂等，重复执行不叠加）。");
        }

        /// <summary>批处理入口：Unity -batchmode -quit -executeMethod CUC260905.Visual.EditorTools.NodeEntranceAnimationSetup.ApplyAll</summary>
        ...
        private static int EnsureComponentOnPrefabRoot(string prefabPath)
        {
            ...
            try
            {
                if (root.GetComponent<NodeEntranceAnimation>() == null)
                {
                    root.AddComponent<NodeEntranceAnimation>();
                    added = 1;
                }
            }
            finally
        ...
        private static int EnsureComponentOnSceneServerNode()
        {
            ...
            if (serverNode.GetComponent<NodeEntranceAnimation>() == null)
            {
                serverNode.AddComponent<NodeEntranceAnimation>();
