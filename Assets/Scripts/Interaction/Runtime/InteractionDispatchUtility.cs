...
namespace CUC260905.Interaction
{
    ...
    public interface IInteractionIntentSink<TIntent>
    ...
    public interface IInteractionIntentSinkResolverUtility : IUtility
    {
        bool TryResolve<TIntent>(
            IInteractionTarget target,
            out IInteractionIntentSink<TIntent> sink)
            where TIntent : struct, IInteractionIntent;
    }

    /// <summary>对解释层暴露统一意图出口，对外提供可观察的调度结果。</summary>
    public interface IInteractionDispatchUtility : IInteractionIntentEmitter, IUtility
    {
    ...
    /// </summary>
    public sealed class InteractionDispatchUtility : IInteractionDispatchUtility
    {
        private readonly IInteractionIntentSinkResolverUtility mSinkResolver;

        public InteractionDispatchUtility(IInteractionIntentSinkResolverUtility sinkResolver)
        {
            ...
            where TIntent : struct, IInteractionIntent
        {
            ...
            if (!mSinkResolver.TryResolve(target, out IInteractionIntentSink<TIntent> sink) || sink == null)
            {
