...
namespace CUC260905.Network
{
    ...
    /// 采样范围从 [InnerRadius, InnerRadius] 起始、随已生成数量按超线性外扩（指数 >1，中心更密）到
    /// [InnerRadius, RangeRadius]，实现"从中心逐步外扩"；InnerRadius 默认 0，允许填充圆心空余，
    /// 使布局更紧凑。构造时对所有数值做钳制与归序。
    public readonly struct UserNodeScatterConfig
    {
        ...
        /// <summary>候选点距原点允许的最小距离（圆盘内半径；0 表示允许填充圆心，布局更紧凑）。</summary>
        ...
        /// <summary>常用默认参数：半径 10、最小距离 0.5、中心留空 0（允许填充圆心）、数量 40–80。</summary>
        public static UserNodeScatterConfig Default
        {
            get
            {
                return new UserNodeScatterConfig(10.0f, 0.5f, 0.0f, 40, 80);
            }
        }
